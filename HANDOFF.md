...
**Branch / commit:** `main` (v0.5.0 merged from `visual-redesign`, PR #2)  
**Latest tag:** `v0.5.0`  
**Current task:** [`NEXT.md`](NEXT.md) — WebdriverIO real-app harness (build on `scripts/app-drive.mjs`)  
**Status / plan:** [`ROADMAP.md`](ROADMAP.md) — **only** place milestone status lives

> **v0.5.0 hard-won terminal facts** (don't relearn these):
> - Real-app verification is mandatory for terminal/TUI work — the mock
>   harness shipped invisible regressions twice. Use `scripts/app-drive.mjs`
>   with `tauri dev` (docs/QA.md §1a); zoom pixel crops when judging glyphs.
> - xterm 6's core renderer has NO custom-glyph drawing; block elements come
>   from the font. `Tethra Blocks` (generated by `scripts/gen-blocks-font.py`,
>   leads the stack in `terminalFontStack()`) inks U+2580–259F full-cell.
>   TrueType LSB must equal each glyph's xMin or WebKit shifts the outline.
> - WebGL addon is unusable in WKWebView: only the first context after page
>   load ever paints. Don't retry; `shouldTryWebgl()` blocks it on purpose.
> - FitAddon subtracts padding only from the `.xterm` element — terminal
>   padding lives in CSS on `.xterm`, never on the host div (clips rows).
> - tmux attach replays junk into scrollback under full-screen agents;
>   `scheduleAttachSweep` (registry.ts) wipes it once post-attach.
