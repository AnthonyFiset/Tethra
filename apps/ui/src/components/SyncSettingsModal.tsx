...
export function SyncSettingsModal({
  ...
  async function saveHttp(): Promise<void> {
    ...
    setMessage(undefined);
    try {
      const next = await syncConfigureHttp(httpUrl.trim(), httpToken.trim() || undefined);
      setStatus(next);
      setMode("http");
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <div className="rounded-md border border-line bg-base px-3 py-2.5 text-micro text-fg-muted">
          ...
          {status?.detail && (
            <div className="mt-1 break-all font-mono text-fg-subtle">{status.detail}</div>
          )}
          {status?.lastSyncedAt && (
        ...
        {mode === "http" ? (
          <div className="flex flex-col gap-3">
            ...
            <Button variant="primary" disabled={busy || !httpUrl.trim()} onClick={() => void saveHttp()}>
              Save HTTP sync
            </Button>
          </div>
        ) : (
